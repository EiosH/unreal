...
class World {
  constructor() {
    ...
    this.points = [];
    this.sticks = [];
    this.createRagdoll(1, 100, 0);
    this.createRagdoll(2, 400, 0);
  ...
  createRagdoll(id, x0, y0) {
    ...
    // Knees
    this.points.push(new Point(x0 + 20, y0 + 130, false, 0.5));
    this.points.push(new Point(x0 - 20, y0 + 130, false, 0.5));

    // Feet
    this.points.push(new Point(x0 + 30, y0 + 180, true, 1));
    this.points.push(new Point(x0 - 0, y0 + 180, true, 1));
    ...
    this.points.slice(i0).map((point) => {
      point.id = id;
    });

    this.sticks.slice(i0).map((stick) => {
      stick.id = id;
    });
  }
  ...
  getClosestPoint(p0, exceptSelf = false) {
    var index = 0;

    this.points
      .map((p1) => this.distance(p0, p1))
      ?.reduce((prev, curr, i) => {
        if (exceptSelf) {
          return prev;
        }
  ...
  collision(p0) {
    var p1 = this.getClosestPoint(p0, true);
    if (p1 && p1.id != p0.id) {
      var dist = this.distance(p0, p1);

      if (dist < 50) {
        console.log(p1);
        if (p0.x + p0.r < p1.x - p1.r) {
          p0.x = p0.x - (p0.r + p1.r);
        }
